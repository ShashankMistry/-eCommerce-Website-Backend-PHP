...
elseif (strpos($endpoint, '/delete/cart') !== false && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $JSON = file_get_contents('php://input');
    $data = json_decode($JSON, true);
    $_POST = $data;
    $stmt = $conn->prepare("DELETE FROM cart WHERE productId = :productId AND email = :email");
    $stmt->bindParam(':productId', $_POST['productId']);
    $stmt->bindParam(':email', $_POST['email']);
    $stmt->execute();
    echo json_encode("Record deleted successfully");
}
//update cart using post method
elseif (strpos($endpoint, '/update/cart') !== false && $_SERVER['REQUEST_METHOD'] == 'POST') {
    $JSON = file_get_contents('php://input');
    $data = json_decode($JSON, true);
    $_POST = $data;
